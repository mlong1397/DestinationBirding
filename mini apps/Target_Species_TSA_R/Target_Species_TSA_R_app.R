...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("species", "Select Bird Species", choices = unique(tsa_bird_data$common_name)),
      imageOutput("bird_image")
    ),
    
    # CHOOSE MAIN PANEL LAYOUT
    mainPanel(
...
server <- function(input, output) {
  ...
  observeEvent(input$species, {
    ...
    target_species_df <- tsa_bird_data[tsa_bird_data$common_name == target_species, ]
